...
public class PersonErrorJobMapper extends Mapper<Object, Text, Text, IntWritable> {
    ...
    public void map(Object key, Text value, Context context)
        ...
        if (anyErrors(splitData[6], splitData[7]) == 1 || anyErrors(splitData[8], splitData[9]) == 1) {
            context.write(new Text(splitData[0]), new IntWritable(1));
        } else {
            context.write(new Text(splitData[0]), new IntWritable(0));
        }
